...
const pickZone = (opts, frameBounds, targetBounds, size) => {
  ...
  const f = frameBounds
  const zones = [
    { side: `start`, standing: `above`, flow: `column`, order: -1, w: (f.x2 - f.x), h: (t.y - f.y) },
    { side: `end`, standing: `right`, flow: `row`, order: 1, w: (f.x2 - t.x2), h: (f.y2 - f.y) },
    { side: `end`, standing: `below`, flow: `column`, order: 1, w: (f.x2 - f.x), h: (f.y2 - t.y2) },
    { side: `start`, standing: `left`, flow: `row`, order: -1, w: (t.x - f.x), h: (f.y2 - f.y) },
  ]
